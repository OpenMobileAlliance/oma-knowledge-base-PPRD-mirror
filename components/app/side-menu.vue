        ...
        <ul class="border-r-2 border-primary/[0.2]">
          <li v-for="(link, index) in items" class="" :key="index">
            <Disclosure v-slot="{ open }" defaultOpen>
              <DisclosureButton
                class="flex w-full justify-between rounded-lg pl-2 py-1 text-left text-sm font-medium text-primary-900 hover:font-bold focus:outline-none focus-visible:ring focus-visible:ring-primary-500/75">
                <ULink :to="link._path" class="no-underline hover:uppercase" :class="isActive(link._path)">{{ link.title
                  }}</ULink>
              </DisclosureButton>
                    ...
                    <Disclosure v-slot="{ open }" defaultOpen v-if="subLink._path !== link._path">
                      <DisclosureButton
                        ...
                        <ULink v-if="link._path !== subLink._path" :to="subLink._path"
                          class="no-underline hover:uppercase" :class="isActive(subLink._path)">
                          {{ subLink.title }}
                        </ULink>
                      </DisclosureButton>
                            ...
                            <Disclosure v-slot="{ open }" defaultOpen v-if="subLink._path !== subChildLink._path">
                              <DisclosureButton
                                ...
                                <ULink v-if="subLink._path !== subChildLink._path" :to="subChildLink._path"
                                  class="no-underline hover:uppercase" :class="isActive(subChildLink._path)">
                                  {{ subChildLink.title }}
                                </ULink>
                              </DisclosureButton>
                                ...
                                <ul v-if="subChildLink.children?.length > 0" class="">
                                  <li v-for="(subSubChildLink, subSubChildIndex) in subChildLink.children" class="py-1"
                                    ...
                                    <ULink v-if="subChildLink._path !== subSubChildLink._path"
                                      :to="subSubChildLink._path" class="pl-2 no-underline hover:uppercase"
